...
use crate::utils::ZeroVec;

#[derive(Default)]
pub(crate) struct FastLocalMoving<T>
where
T: Float + FromPrimitive + ToPrimitive + Send + Sync + Sum + MulAssign + std::ops::AddAssign + Default {
    resolution: T,
...
where
        ...
        R: RngCore,
    {
        ...
        while num_unstable_nodes > 0 {
            ...
            self.cluster_weights[current_cluster] = self.cluster_weights[current_cluster] - network.weight(j);
